...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (product) => {
    try {
      ...
      const response = await apiService.post('/cart', cartItem);
      if (response.data?.items) {
        setCart(response.data.items);
  ...
  const updateQuantity = async (itemId, quantity) => {
    try {
      ...
      // Use the product ID in the URL as per backend API
      const response = await apiService.put(`/cart/${cartItem.product._id}`, { 
        quantity 
      });
