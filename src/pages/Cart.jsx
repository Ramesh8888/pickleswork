...
const Cart = () => {
  ...
  const handleQuantityChange = async (itemId, newQuantity) => {
    try {
      ...
      if (newQuantity < (item.product?.minOrder || 1) || newQuantity > (item.product?.maxOrder || 100)) {
        toast.error(`Quantity must be between ${item.product?.minOrder || 1} and ${item.product?.maxOrder || 100}`);
        return;
      }
